...
import { ShieldCheck, Camera, CheckCircle2, Lock } from "lucide-react";

// 데모용 데이터 (실제로는 DB에서 불러옴)
const partnerData = {
  name: "공간작업소",
  completed_count: 12, // 10건 이상
  dispute_count: 0,    // 분쟁 0건
  real_portfolio: [    // 보정 없는 현장 사진들
...
export default function PartnerProfile() {
  ...
  return (
    <div className="min-h-screen bg-background p-4 space-y-6">
          ...
          {isSafeMaster && (
            <div className="flex flex-col items-center">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center shadow-lg shadow-amber-500/30">
                ...
                <span className="absolute -bottom-1 bg-white text-amber-600 text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow">
                  MASTER
                </span>
              </div>
              안심 마스터
      ...
      {/* --- Feature 2: 리얼 포트폴리오 (Raw Portfolio) --- */}
      <Card className="p-5 border-0 shadow-lg">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          업체가 대금 지급을 위해 올린 100% 실제 현장 증빙 사진입니다.
        </p>
          ...
          {partnerData.real_portfolio.map((item, idx) => (
            <div key={idx} className="relative aspect-square rounded-lg overflow-hidden group">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-2">
                <Badge variant="secondary" className="w-fit text-[10px] bg-white/20 text-white border-0 mb-1">
                  Raw Data
                </Badge>
